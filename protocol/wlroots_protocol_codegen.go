...
// Package wlroots_protocol defines extension modules for the Soong build system
// to make it easier to generate code from a list of Wayland protocol files.
//
// The primary extension module is "wlroot_protocol_codegen", which applies a
...
func init() {
	...
	android.RegisterModuleType(
		"wlroots_protocol_codegen", wlrootsCodegenModuleFactory)
}

var (
	// Create a context for build rule output from this package
	pctx = android.NewPackageContext("android/soong/external/wlroots-master/protocol")
)
...
// wlrootsCodegenProperties defines the properties that will be read in from the
// Android.bp file for each instantiation of the module.
type wlrootsCodegenProperties struct {
	// This string gives the command line template to run on each protocol file
	// to wlroots_protocol_codegen.
	//
...
// wlrootsGenModule defines the Soong module for each instance.
type wlrootsGenModule struct {
	android.ModuleBase

	// Store a copy of the parsed properties for easy reference.
	properties wlrootsCodegenProperties
...
// an interface. In this case we want to be sure that wlrootsGenModule
// implements genrule.SourceFileGenerator
var _ genrule.SourceFileGenerator = (*wlrootsGenModule)(nil)

// Check that we implement android.SourceFileProducer
var _ android.SourceFileProducer = (*wlrootsGenModule)(nil)
...
func (g *wlrootsGenModule) GeneratedSourceFiles() android.Paths {
...
// directories.
func (g *wlrootsGenModule) GeneratedHeaderDirs() android.Paths {
...
func (g *wlrootsGenModule) GenerateAndroidBuildActions(ctx android.ModuleContext) {
...
func (g *wlrootsGenModule) generateOutputPath(ctx android.ModuleContext, src android.Path, prefix string, suffix string) android.WritablePath {
	// Construct a new filename by adding the requested prefix and suffix for this
	// code generator instance. If the input file name is "wlroots.xml", and the
	// properties specify a prefix of "test-" and a suffix of "-client.cpp", we
	// will end up with a fulename of "test-wlroots-client.cpp"
...
func (g *wlrootsGenModule) emitRule(ctx android.ModuleContext, tools map[string]android.Path) {
...
func (g *wlrootsGenModule) emitBuild(ctx android.ModuleContext, src android.Path, out android.WritablePath, implicitDeps android.Paths) android.Path {
...
func (g *wlrootsGenModule) prepareTools(ctx android.ModuleContext) (tools map[string]android.Path, implicitDeps android.Paths) {
...
// an error value.
func (g *wlrootsGenModule) expandCmd(ctx android.ModuleContext, tools map[string]android.Path) (cmd string) {
...
// wlrootsCodegenModuleFactory creates an extension module instance.
func wlrootsCodegenModuleFactory() android.Module {
	m := &wlrootsGenModule{}
